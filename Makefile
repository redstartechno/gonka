...
GHCR_CACHE_OWNER ?= gonka-ai
PLATFORM ?= linux/amd64
GOOS ?= linux
GOARCH ?= amd64
ifeq ($(USE_REGISTRY_CACHE),1)
_MOCK_CACHE_ARGS := --cache-from type=registry,ref=ghcr.io/$(GHCR_CACHE_OWNER)/mock-server:buildcache --cache-to type=registry,ref=ghcr.io/$(GHCR_CACHE_OWNER)/mock-server:buildcache,mode=min
_MOCK_BUILD_CMD := docker buildx build --load $(_MOCK_CACHE_ARGS)
_DEVSHARDD_CACHE_ARGS := --cache-from type=registry,ref=ghcr.io/$(GHCR_CACHE_OWNER)/devshardd:buildcache --cache-to type=registry,ref=ghcr.io/$(GHCR_CACHE_OWNER)/devshardd:buildcache,mode=min
