...
func TestApplyTestnetOnlyOverrides_UpsertsDevshardVersionAndPreservesFees(t *testing.T) {
	k, ctx, _ := keepertest.InferenceKeeperReturningMocks(t)
	...
	require.Len(t, got.DevshardEscrowParams.ApprovedVersions, 1)
	version := got.DevshardEscrowParams.ApprovedVersions[0]
	require.Equal(t, "v1", version.Name)
	require.Equal(t,
		"https://github.com/product-science/race-releases/releases/download/release%2Fv0.2.12-testnet16/devshardd.zip",
		version.Binary,
	)
	require.Equal(t, "a21628eae05f3d9b09afaa67043fee3f743f46efb50a9ce687456fc4f1451706", version.Sha256)

	defaultFeeParams := inferencetypes.DefaultFeeParams()
	require.NotNil(t, got.FeeParams)
	require.Equal(t, defaultFeeParams.MinGasPriceNgonka, got.FeeParams.MinGasPriceNgonka)
	require.Equal(t, defaultFeeParams.BaseValidationGas, got.FeeParams.BaseValidationGas)
	require.Equal(t, defaultFeeParams.GasPerPocCount, got.FeeParams.GasPerPocCount)
}
